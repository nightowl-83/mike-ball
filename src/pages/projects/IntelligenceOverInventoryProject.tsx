...
const IntelligenceOverInventoryProject = () => {
  ...
  const sectionData = [{
    ...
    label: 'The Challenge',
    number: '/01',
    subtitle: 'Understanding the commodity data problem'
  }, {
    id: 'idea',
    label: 'The Idea',
    number: ''
    ...
    label: 'The Solution',
    number: '/02',
    subtitle: 'Building the data pipeline'
  }, {
    id: 'parsing',
    label: 'Parsing Tool',
    number: '/03',
    subtitle: 'Extracting buyer intent from leads'
  }, {
    id: 'impact',
    label: 'Impact',
    number: '/04',
    subtitle: 'Dual-interface implementation'
  }, {
    id: 'strategy',
    label: 'Strategy',
    number: '/05',
    subtitle: 'Influencing product direction'
  }, {
    id: 'gallery',
    label: 'Gallery',
    number: '',
    subtitle: 'Messaging and treatment variations'
  }, {
    id: 'vision',
    label: 'Vision',
    number: '/06',
  ...
  const flowSteps = [{
    icon: Database,
    label: "Lead Data",
    description: "Capture buyer inquiries and engagement signals"
  }, {
    icon: Filter,
    label: "Keyword Parser",
    description: "Extract intent keywords from lead messages"
  }, {
    icon: Target,
    label: "Intent Mapping",
    description: "Map keywords to filter categories"
  }, {
    icon: BarChart3,
    label: "UI Filters",
    description: "Surface as advanced search options"
  }, {
    icon: Users,
    label: "Seller Training",
    description: "Provide data for what buyers are searching for"
  }];
  ...
  const parsingCardPairs = [{
    ...
    highlights: ["Natural language processing for lead content", "Trend tracking over configurable time periods", "Exportable reports for stakeholder review"]
  }, {
    images: [{
      src: trendiAdvanced,
  ...
  const challengePoints = [{
    ...
    text: "While we maintained a strong core of unique listings, the majority of our traffic was driven by third-party feeds. Because these same base listings appeared on every competing platform, new-to-market products could easily chip away at our traffic by offering similar utility with less overhead."
  }, {
    title: "The Insight Gap",
    text: "We knew people were clicking, but we didn't know why. Our engagement data was surface-level—we could see that a listing was popular, but we couldn't see the specific requirements or 'deal-breakers' that buyers were looking for within the data."
  }, {
    title: "The Seller Disconnect",
    text: "Sellers were receiving vanity metrics like total views and saves. These numbers looked good on a report but offered zero guidance on how to actually close a deal. A seller with 1,000 views and zero leads had no way to know if their price was wrong or if they were missing a key piece of information."
  }, {
    title: "Database-First Filtering",
    text: "Our search filters were limited by the technical constraints of the incoming feeds. We were forcing buyers to search based on how the database was structured, rather than the natural language of land buying (e.g., 'Is there power at the road?' or 'Does it have a well?')."
  }];
  ...
  useEffect(() => {
    ...
    challengeSection.addEventListener('wheel', handleWheel, {
      passive: false
    });
  ...
  useEffect(() => {
      ...
      if (delta > 0) {
        if (parsingWheelAccumulatorRef.current >= WHEEL_THRESHOLD) {
          if (parsingActiveIndex < parsingCardPairs.length - 1) {
            ...
            parsingWheelAccumulatorRef.current = 0;
            setTimeout(() => {
              parsingIsTransitioningRef.current = false;
            }, 400);
          }
        } else {
          e.preventDefault();
    ...
    parsingSection.addEventListener('wheel', handleWheel, {
      passive: false
    });
  ...
  useEffect(() => {
      ...
      if (delta > 0) {
        if (strategyWheelAccumulatorRef.current >= WHEEL_THRESHOLD) {
          if (strategyActiveIndex < strategyItems.length - 1) {
            ...
            strategyWheelAccumulatorRef.current = 0;
            setTimeout(() => {
              strategyIsTransitioningRef.current = false;
            }, 400);
          }
        } else {
          e.preventDefault();
    ...
    strategySection.addEventListener('wheel', handleWheel, {
      passive: false
    });
  ...
  return <div className="flex bg-background">
      ...
      <main ref={containerRef} className={cn("flex-1 slide-container transition-colors duration-500", "ml-0 md:ml-56 lg:ml-64", "pt-14 md:pt-0", "bg-background", activeDataMode === 'marketplace' && "theme-light")}>
          ...
          <div className="w-[85%] mx-auto py-12">
            <div className="space-y-6 max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-wrap justify-center gap-3 pt-4 animate-fade-in [animation-delay:500ms]">
                <span className="px-4 py-2 rounded-full border border-primary/20 text-base font-normal bg-[#3835e3]/20 text-[#7a78e8]">First-Party Data  ·  Product Strategy</span>
              </div>
            </div>
          </div>
        </section>
        ...
        (challengeSectionRef as any).current = el;
          ...
          <div className="w-[85%] mx-auto">
            ...
            <div className="max-w-4xl space-y-8">
              {challengePoints.map((point, index) => <div key={index} className={cn("transition-all duration-500", challengeActiveIndex === index ? "opacity-100" : "opacity-40")}>
                  <h3 className={cn("text-base md:text-lg font-semibold mb-2 transition-colors duration-500", challengeActiveIndex === index ? "text-primary" : "text-muted-foreground")}>
                    {point.title}
                  </h3>
                  <p className={cn("text-lg md:text-xl leading-relaxed transition-all duration-500", challengeActiveIndex === index ? "text-foreground" : "text-muted-foreground/70")}>
                    {point.text}
                  </p>
                </div>)}
            </div>
            ...
            <div className="flex gap-2 mt-10">
              {challengePoints.map((_, index) => <button key={index} onClick={() => setChallengeActiveIndex(index)} className={cn("w-2 h-2 rounded-full transition-all duration-300", "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background", challengeActiveIndex === index ? "bg-primary w-6" : "bg-muted-foreground/30 hover:bg-muted-foreground/50")} aria-label={`Go to challenge point ${index + 1} of ${challengePoints.length}`} />)}
            </div>
          </div>

          {/* Navigation Arrows - Bottom Right */}
          <div className="absolute bottom-8 right-8 flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={() => setChallengeActiveIndex(prev => Math.max(0, prev - 1))} disabled={challengeActiveIndex === 0} className="h-10 w-10">
              <ChevronLeft className="w-5 h-5" />
            </Button>
            <Button variant="outline" size="icon" onClick={() => setChallengeActiveIndex(prev => Math.min(challengePoints.length - 1, prev + 1))} disabled={challengeActiveIndex === challengePoints.length - 1} className="h-10 w-10">
              <ChevronRight className="w-5 h-5" />
            </Button>
            <Button variant="outline" onClick={() => scrollToSection(2)} className="gap-2 ml-2">
              Continue
        ...
        (sectionRefs[2] as any).current = el;
          ...
          <div className="absolute left-1/2 -translate-x-1/2 bottom-0 w-0.5 h-32" style={{
        ...
        <section ref={el => {
        (sectionRefs[3] as any).current = el;
      }} className="slide-section flex items-center justify-center relative">
          {/* Full viewport height gradient line - CENTERED on page */}
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-0.5 z-0" style={{
          background: `linear-gradient(
                to bottom,
                hsl(var(--background)) 0%,
                hsl(var(--border)) 15%,
          ...
          <div className="w-full px-4 md:px-8 lg:px-12 relative z-10">
            ...
            <div className="max-w-2xl mx-auto space-y-4 relative">
              {flowSteps.map((step, index) => <div key={step.label} className="relative">
                  {/* Card */}
        ...
        (parsingSectionRef as any).current = el;
                      ...
                      <div className="space-y-6 max-w-lg">
                        ...
                        <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 space-y-4">
                          ...
                          <ul className="space-y-3">
                            {pair.highlights.map((highlight, idx) => <li key={idx} className="flex items-start gap-3 text-base text-muted-foreground">
          ...
          {currentSectionIndex === 4 && <div className="fixed bottom-8 right-8 flex items-center gap-2 z-50 md:flex hidden">
              <span className="text-sm text-muted-foreground mr-2">{parsingActiveIndex + 1}/{parsingCardPairs.length}</span>
              <Button variant="outline" size="icon" onClick={() => setParsingActiveIndex(prev => Math.max(0, prev - 1))} disabled={parsingActiveIndex === 0} className="h-10 w-10">
                <ChevronLeft className="w-5 h-5" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => setParsingActiveIndex(prev => Math.min(parsingCardPairs.length - 1, prev + 1))} disabled={parsingActiveIndex === parsingCardPairs.length - 1} className="h-10 w-10">
                <ChevronRight className="w-5 h-5" />
              </Button>
              <Button variant="outline" onClick={() => scrollToSection(5)} className="gap-2 ml-2">
                Continue
                <ArrowDown className="w-4 h-4" />
              </Button>
            </div>}
        ...
        <ImpactSection04 sectionRef={el => {
        (sectionRefs[5] as any).current = el;
      }} activeTab={activeDataMode} setActiveTab={setActiveDataMode} scrollToSection={scrollToSection} isActive={currentSectionIndex === 5} />
        ...
        (strategySectionRef as any).current = el;
          ...
          {/* Navigation Arrows - Only visible when on this section */}
          {currentSectionIndex === 6 && <div className="fixed bottom-8 right-8 flex items-center gap-2 z-50 md:flex hidden">
              <span className="text-sm text-muted-foreground mr-2">{strategyActiveIndex + 1}/{strategyItems.length}</span>
              <Button variant="outline" size="icon" onClick={() => setStrategyActiveIndex(prev => Math.max(0, prev - 1))} disabled={strategyActiveIndex === 0} className="h-10 w-10">
                <ChevronLeft className="w-5 h-5" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => setStrategyActiveIndex(prev => Math.min(strategyItems.length - 1, prev + 1))} disabled={strategyActiveIndex === strategyItems.length - 1} className="h-10 w-10">
                <ChevronRight className="w-5 h-5" />
              </Button>
              <Button variant="outline" onClick={() => scrollToSection(7)} className="gap-2 ml-2">
                Continue
                <ArrowDown className="w-4 h-4" />
              </Button>
            </div>}
        ...
        <section ref={el => {
        (sectionRefs[8] as any).current = el;
        ...
        {/* Next Project - /07 */}
        <section ref={el => {
        (sectionRefs[9] as any).current = el;
