...
export const ScrollStorySection = ({
  ...
  return <section ref={containerRef} className={cn('relative', className)} style={{
    height: containerHeight
  }}>
      {/* Sticky container that holds both columns */}
      <div className="sticky top-0 h-screen overflow-hidden">
        {/* Optional Section Header with Progress Indicator */}
        {(sectionTitle || sectionNumber) && <div className="absolute top-0 left-0 right-0 z-10 pt-20 md:pt-24">
            <div className="container mx-auto max-w-[1440px] px-6 md:px-12 lg:px-20">
              <div className="flex items-baseline justify-between mb-6">
                {sectionTitle && <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-foreground">
                    {sectionTitle}
                  </h2>}
                {sectionNumber && <span className="text-xl md:text-4xl font-bold font-mono opacity-20">
                    {sectionNumber}
                  </span>}
              </div>
        ...
        <div className={cn('h-full flex flex-col md:flex-row', reversed ? 'md:flex-row-reverse' : '')}>
          {/* Text Column */}
              ...
              return <div key={index} className={cn('absolute inset-0 flex flex-col justify-center transition-all duration-700 ease-out', isActive ? 'opacity-100 translate-y-0' : 'opacity-0', isPast ? '-translate-y-12' : 'translate-y-12')} style={{
                pointerEvents: isActive ? 'auto' : 'none'
                    ...
                    <p className="text-lg md:text-xl lg:text-2xl leading-relaxed">
                      {words.map((word, wordIndex) => <span key={wordIndex} className={cn('inline-block mr-[0.3em] transition-all ease-out', isActive ? 'opacity-100 translate-y-0 blur-0' : 'opacity-0 translate-y-4 blur-sm')} style={{
                    transitionDuration: '600ms',
                    transitionDelay: isActive ? `${wordIndex * 30}ms` : '0ms'
                  }}>
                          <span className={cn('transition-colors duration-500', isActive ? 'text-foreground' : 'text-muted-foreground')}>
                            {word}
                          </span>
                        </span>)}
                    </p>
          ...
          <div className="w-full md:w-1/2 h-1/2 md:h-full relative bg-muted/30 overflow-hidden">
            ...
            return <div key={index} className={cn('absolute inset-0 transition-all duration-700 ease-out', isActive ? 'opacity-100 z-10' : 'opacity-0 z-0')} style={{
              transform: `scale(${scale})`
            }}>
                  <img src={story.image} alt={story.imageAlt} className="w-full h-full object-cover object-center" loading="lazy" />
                </div>;
          })}
          </div>
        </div>
